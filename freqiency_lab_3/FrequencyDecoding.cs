...
{
    internal class FrequencyDecoding
    {
        ...
        public static List<(char letter1, char letter2)> ListLettersChanged(in string path, in string lang)
        {
            ...
            var frequencyTable = FrequencyTable.FrequencyTableList(path, lang).OrderByDescending(i=>i.CalcPercent).Select(i=>i.Symbol).ToList();
            for (int i = 0; i < FrequencyTable.standartFrequencyTable[lang].Count(); i++)
            {
                letters.Add((frequencyTable[i], FrequencyTable.standartFrequencyTable[lang][i].Symbol));
            }
            return letters;
        }
